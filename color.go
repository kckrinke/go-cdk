...
// Stringer interface implementation so the Color can be used in normal fmt
// operations without required the developer to jump any hoops.
func (c Color) String() string {
	...
	if c.Hex() >= 0 {
		return fmt.Sprintf("%v[#%0.6x]", name, c.Hex())
	}
	return fmt.Sprintf("%v[%v]", name, c.Hex())
}
