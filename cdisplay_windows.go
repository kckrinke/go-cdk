...
func (s *cDisplay) CharacterSet() string {
	// We are always UTF-16LE on Windows
	return "UTF-16LE"
}

func (s *cDisplay) EnableMouse(...MouseFlags) {
	s.setInMode(modeResizeEn | modeMouseEn | modeExtndFlg)
}

func (s *cDisplay) DisableMouse() {
	s.setInMode(modeResizeEn | modeExtndFlg)
}

func (s *cDisplay) EnablePaste() {}

func (s *cDisplay) DisablePaste() {}

func (s *cDisplay) Fini() {
	s.finishOnce.Do(s.finish)
}

func (s *cDisplay) finish() {
	s.Lock()
	s.style = StyleDefault
	s.cursorX = -1
	s.cursorY = -1
...
func (s *cDisplay) PostEventWait(ev Event) {
	s.evCh <- ev
}

func (s *cDisplay) PostEvent(ev Event) error {
	select {
	case s.evCh <- ev:
		return nil
	default:
...
func (s *cDisplay) PollEvent() Event {
	select {
	case <-s.quit:
		return nil
	case ev := <-s.evCh:
...
func (s *cDisplay) ShowCursor(x, y int) {
	s.Lock()
	if !s.finished {
		s.cursorX = x
		s.cursorY = y
...
func (s *cDisplay) getConsoleInput() error {
...
func (s *cDisplay) mapStyle(style Style) uint16 {
...
func (s *cDisplay) SetCell(x, y int, style Style, ch ...rune) {
	if len(ch) > 0 {
...
func (s *cDisplay) SetContent(x, y int, mc rune, comb []rune, style Style) {
	s.Lock()
	if !s.finished {
		s.cells.SetContent(x, y, mc, comb, style)
	}
	s.Unlock()
}

func (s *cDisplay) GetContent(x, y int) (rune, []rune, Style, int) {
	s.Lock()
	mc, comb, style, width := s.cells.GetContent(x, y)
	s.Unlock()
	return mc, comb, style, width
}

func (s *cDisplay) sendVtStyle(style Style) {
	esc := &strings.Builder{}
...
func (s *cDisplay) writeString(x, y int, style Style, ch []uint16) {
...
func (s *cDisplay) draw() {
	...
	for y := 0; y < s.h; y++ {
		for x := 0; x < s.w; x++ {
			...
			if len(comb) != 0 {
				wcs = append(wcs, utf16.Encode(comb)...)
			}
...
func (s *cDisplay) getConsoleInfo(info *consoleInfo) {
	procGetConsoleScreenBufferInfo.Call(
		uintptr(s.out),
		uintptr(unsafe.Pointer(info)))
}

func (s *cDisplay) getCursorInfo(info *cursorInfo) {
	procGetConsoleCursorInfo.Call(
		uintptr(s.out),
		uintptr(unsafe.Pointer(info)))
}

func (s *cDisplay) setCursorInfo(info *cursorInfo) {
...
func (s *cDisplay) setCursorPos(x, y int) {
	if s.vTen {
...
func (s *cDisplay) setBufferSize(x, y int) {
	procSetConsoleScreenBufferSize.Call(
		uintptr(s.out),
		coord{int16(x), int16(y)}.uintptr())
}

func (s *cDisplay) Size() (int, int) {
	s.Lock()
	w, h := s.w, s.h
	s.Unlock()
...
func (s *cDisplay) Clear() {
	s.Fill(' ', s.style)
}

func (s *cDisplay) Fill(r rune, style Style) {
	s.Lock()
	if !s.finished {
...
func (s *cDisplay) clearScreen(style Style) {
	if s.vTen {
		s.sendVtStyle(style)
...
func (s *cDisplay) setInMode(mode uint32) error {
...
func (s *cDisplay) setOutMode(mode uint32) error {
	rv, _, err := procSetConsoleMode.Call(
		uintptr(s.out),
		uintptr(mode))
	if rv == 0 {
...
func (s *cDisplay) getInMode(v *uint32) {
	procGetConsoleMode.Call(
		uintptr(s.in),
		uintptr(unsafe.Pointer(v)))
}

func (s *cDisplay) getOutMode(v *uint32) {
	procGetConsoleMode.Call(
		uintptr(s.out),
		uintptr(unsafe.Pointer(v)))
}

func (s *cDisplay) SetStyle(style Style) {
	s.Lock()
	s.style = style
	s.Unlock()
}
...
func (s *cDisplay) RegisterRuneFallback(r rune, subst string) {
}

func (s *cDisplay) UnregisterRuneFallback(r rune) {
}

func (s *cDisplay) CanDisplay(r rune, checkFallbacks bool) bool {
...
func (s *cDisplay) HasMouse() bool {
	return true
}

func (s *cDisplay) Resize(int, int, int, int) {}

func (s *cDisplay) HasKey(k Key) bool {
