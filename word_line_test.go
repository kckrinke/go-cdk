...
func TestWordLine(t *testing.T) {
	Convey("Word Lines with...", t, func() {
		...
		Convey("Empty Strings", func() {
			wl := NewWordLine("", DefaultMonoStyle)
			So(wl, ShouldNotBeNil)
		...
		Convey("One Word", func() {
			wl := NewWordLine("word", DefaultMonoStyle)
			So(wl, ShouldNotBeNil)
			So(wl.Value(), ShouldEqual, "word")
			So(wl.Words(), ShouldHaveLength, 1)
			So(wl.CharacterCount(), ShouldEqual, 4)
			So(wl.WordCount(), ShouldEqual, 1)
			err := wl.AppendWordRune(0, '!', DefaultMonoStyle)
			So(err, ShouldBeNil)
			So(wl.Value(), ShouldEqual, "word!")
			err = wl.AppendWordRune(1, '#', DefaultMonoStyle)
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldEqual, "word at index 1 not found")
		})
		Convey("More Than One Word", func() {
			wl := NewWordLine("more than words", DefaultMonoStyle)
			So(wl, ShouldNotBeNil)
		...
		Convey("Making things...", func() {
			Convey("wrapping on...", func() {
				Convey("word", func() {
					wl := NewWordLine("one two three", DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines := wl.Make(false, WRAP_WORD, false, JUSTIFY_LEFT, 10, DefaultMonoStyle)
					So(lines, ShouldHaveLength, 2)
					So(lines[0].Len(), ShouldEqual, 4)
					So(lines[1].Len(), ShouldEqual, 1)
					wl = NewWordLine("one two three\nfour five six", DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines = wl.Make(false, WRAP_WORD, false, JUSTIFY_LEFT, 10, DefaultMonoStyle)
					...
					wl = NewWordLine("1234567890ABCDEF", DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines = wl.Make(false, WRAP_WORD, false, JUSTIFY_LEFT, 10, DefaultMonoStyle)
					...
					wl = NewWordLine("1234567890   DEF", DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines = wl.Make(false, WRAP_WORD, false, JUSTIFY_LEFT, 10, DefaultMonoStyle)
					So(lines, ShouldHaveLength, 2)
					So(lines[0].Value(), ShouldEqual, "1234567890")
					...
					So(lines[1].Len(), ShouldEqual, 1)
					wl = NewWordLine("1234567890   ABC DEF", DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines = wl.Make(false, WRAP_WORD, false, JUSTIFY_LEFT, 10, DefaultMonoStyle)
					So(lines, ShouldHaveLength, 2)
					So(lines[0].Value(), ShouldEqual, "1234567890")
				...
				Convey("char", func() {
					wl := NewWordLine("one two three", DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines := wl.Make(false, WRAP_CHAR, false, JUSTIFY_LEFT, 10, DefaultMonoStyle)
					So(lines, ShouldHaveLength, 2)
				...
				Convey("word-char", func() {
					wl := NewWordLine("one two three", DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines := wl.Make(false, WRAP_WORD_CHAR, false, JUSTIFY_LEFT, 10, DefaultMonoStyle)
					So(lines, ShouldHaveLength, 2)
					So(lines[0].Value(), ShouldEqual, "one two ")
				...
				Convey("none", func() {
					wl := NewWordLine("one two three", DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines := wl.Make(false, WRAP_NONE, false, JUSTIFY_LEFT, 10, DefaultMonoStyle)
					So(lines, ShouldHaveLength, 1)
					So(lines[0].Value(), ShouldEqual, "one two th")
					So(lines[0].CharacterCount(), ShouldEqual, 10)
					So(lines[0].Len(), ShouldEqual, 5)
					...
					So(lines[0].CharacterCount(), ShouldEqual, 10)
					So(lines[0].Len(), ShouldEqual, 1)
					wl = NewWordLine("one two three\n1234567890ABCDEF", DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines = wl.Make(false, WRAP_NONE, false, JUSTIFY_LEFT, 10, DefaultMonoStyle)
					So(lines, ShouldHaveLength, 2)
					So(lines[0].Value(), ShouldEqual, "one two th")
			...
			Convey("aligning to...", func() {
				wl := NewWordLine("one", DefaultMonoStyle)
				So(wl, ShouldNotBeNil)
				lines := wl.Make(false, WRAP_NONE, false, JUSTIFY_LEFT, 10, DefaultMonoStyle)
				So(lines, ShouldHaveLength, 1)
				...
				So(lines[0].Len(), ShouldEqual, 1)
				lines = wl.Make(false, WRAP_NONE, false, JUSTIFY_RIGHT, 10, DefaultMonoStyle)
				So(lines, ShouldHaveLength, 1)
				So(lines[0].Value(), ShouldEqual, "       one")
				So(lines[0].CharacterCount(), ShouldEqual, 10)
				So(lines[0].Len(), ShouldEqual, 8)
				lines = wl.Make(false, WRAP_NONE, false, JUSTIFY_CENTER, 10, DefaultMonoStyle)
				So(lines, ShouldHaveLength, 1)
				So(lines[0].Value(), ShouldEqual, "    one")
				So(lines[0].CharacterCount(), ShouldEqual, 7)
				So(lines[0].Len(), ShouldEqual, 5)
				wl = NewWordLine("one two", DefaultMonoStyle)
				So(wl, ShouldNotBeNil)
				lines = wl.Make(false, WRAP_NONE, false, JUSTIFY_FILL, 10, DefaultMonoStyle)
				So(lines, ShouldHaveLength, 1)
				So(lines[0].Value(), ShouldEqual, "one    two")
				So(lines[0].CharacterCount(), ShouldEqual, 10)
				So(lines[0].Len(), ShouldEqual, 3)
				wl = NewWordLine("1 2 3 4", DefaultMonoStyle)
				So(wl, ShouldNotBeNil)
				lines = wl.Make(false, WRAP_NONE, false, JUSTIFY_FILL, 10, DefaultMonoStyle)
				So(lines, ShouldHaveLength, 1)
